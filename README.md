...
**Любой другой агент** (ChatGPT, Cursor, свой бот и т.п.) — дайте ему такую инструкцию в начале работы:

> Ты ведёшь мой проектный vault по методологии taskvault. Сначала прочитай в корне репозитория `CLAUDE.md` и `_rules.md` — это твои правила. Навыки лежат в `.claude/skills/*/SKILL.md` (`sync`, `backlog`, `close`, `inbox`) — это инструкции, выполняй их по шагам, когда они применимы. Ключевое: каждое значимое действие отражай задачей и не отчитывайся «готово», пока работа не отражена задачей и не проверена. Начни с `sync` — загрузи контекст всех проектов и предложи, с чего начать.

Скопируйте этот абзац агенту — дальше он работает сам. (Файлы `SKILL.md` — обычный markdown, любая модель с доступом к файлам их исполнит.)
