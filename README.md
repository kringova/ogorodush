...
Канонические правила лежат в `AGENTS.md` (вендор-нейтральный формат, который многие инструменты читают сами). Для Claude Code рядом есть `CLAUDE.md` с одной строкой `@AGENTS.md` — он подтягивает тот же файл (Claude Code не читает `AGENTS.md` напрямую, но поддерживает такой импорт).

**Claude Code** — ничего просить не нужно: откройте папку vault, правила придут из `CLAUDE.md` → `AGENTS.md`, навыки (`sync`/`backlog`/`close`/`inbox`) — из `.claude/skills/`. Скажите «sync», чтобы агент загрузил контекст и предложил, с чего начать.

**Любой другой агент** (ChatGPT, Cursor, Codex, свой бот) — многие читают `AGENTS.md` сами; если нет — скопируйте эту инструкцию в начале работы (кнопка копирования — в правом верхнем углу блока):
