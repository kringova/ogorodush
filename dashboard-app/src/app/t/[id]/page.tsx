...
/** Канонический адрес задачи: /t/OGOROD-0042 (или /t/42 → 301 на канон). */
export default async function TaskShortPage({
  ...
  // Резолв по ключу/slug (ручные задачи — ключ = slug), затем по числовому id (/t/42 → OGOROD-…)
  let task = all.find((t) => t.key === id || t.slug === id) ?? null;
  if (!task && Number.isFinite(num)) task = all.find((t) => t.id === num) ?? null;
  if (!task) notFound();

  // канонизируем только агентские (числовой id → /t/OGOROD-0042); ручные остаются на /t/<slug>
